// Topic research for the Content Engine. Scores Search Console queries as
// opportunities, sends the top signals to Claude with the client brand
// context, and returns a prioritized list of content opportunities with
// ranking-aware angles.
//
// This file holds everything except the Search Console fetch, which lives in
// topicResearch.js. Keeping the browser-only Google auth out of here lets the
// server-side Autopilot (netlify/functions/lib/autopilot.js) import the exact
// same research logic under Node. Browser code keeps importing topicResearch.js.
...
// The research payload when Search Console is unavailable: topics are then
// generated from the brand context alone.
export function emptyResearch(days = 90) {
  return {
    days, totalImpressions: 0, totalClicks: 0, siteAvgCtr: 0,
    queries: [], topOpportunities: [], pageByQuery: {}, allQueryCount: 0
  };
}

// queries: rows from a ['query'] Search Analytics call; pageQueries: rows
// from a ['page','query'] call (both already flattened, see gsc.js).
export function summarizeResearch(queries, pageQueries, days = 90) {
...
// `complete` is the Claude call; the server passes its own (API key from the
// environment). The browser uses the default.
export async function generateTopicRecommendations(client, research, { targetArticles, complete = claudeComplete } = {}) {
