...
function buildCombinedAeoHtml(opts) {
  ...
  // ONLY the implementation HTML goes in the visible output. The agent-
  // facing context (AEO type label, opt name, description, "where to
  // place" hint) was being treated as page content by users who pasted
  // the previous version verbatim. Now those live in HTML comments so
  // they're recoverable from page source but never render to the user.
  // For FAQ-style implementations we wrap the whole block in a native
  // <details>/<summary> accordion — AEO-safe because the content is in
  // the page source on initial load (bots see it; humans toggle it).
  const sections = visible
    ...
    .map((o, i) => {
      ...
      const isFaq = /faq|question/i.test(o.type + ' ' + o.name + ' ' + o.title);
      const body = isFaq
        ? wrapFaqAsAccordion(o._code, o.name || 'FAQ')
        : o._code;
      return `
<!--
...
`;
  ...
  Each implementation block is preceded by a comment with type +
  recommended placement so a developer reviewing the page source can
  trace each one back. Strip the comments if you prefer.
-->
${sections}${schemaJoined}`.trim();
}

// Convert an FAQ-style HTML block (h2/h3 questions + p answers) into a
// stack of <details>/<summary> accordions. AEO-safe because the entire
// content is in the rendered HTML on initial load — bots and humans
// both see the full text. The <summary> contains the question; the
// expanded answer is everything between question N and question N+1.
function wrapFaqAsAccordion(html, fallbackTitle) {
  if (!html || !/<h[1-6]\b/i.test(html)) return html;
  const titleMatch = html.match(/^\s*<h[12]\b[^>]*>([\s\S]*?)<\/h[12]>/i);
  const sectionTitle = titleMatch ? stripTags(titleMatch[1]) : fallbackTitle;
  const rest = titleMatch ? html.slice(titleMatch[0].length) : html;
  // Split on h3 boundaries (most common Q level). Fall back to h2.
  const splitRe = /<h3\b[^>]*>([\s\S]*?)<\/h3>/gi;
  let last = 0, m;
  const items = [];
  while ((m = splitRe.exec(rest)) !== null) {
    if (items.length) {
      items[items.length - 1].body = rest.slice(last, m.index).trim();
    }
    items.push({ q: stripTags(m[1]), body: '' });
    last = m.index + m[0].length;
  }
  if (items.length) items[items.length - 1].body = rest.slice(last).trim();
  if (!items.length) return html;

  return `<section class="aeo-faq">
  <h2>${escapeHtmlForBlock(sectionTitle)}</h2>
  ${items.map(it => `<details>
    <summary>${escapeHtmlForBlock(it.q)}</summary>
    ${it.body}
  </details>`).join('\n  ')}
</section>`;
}
