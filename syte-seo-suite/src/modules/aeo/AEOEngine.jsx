...
function buildCombinedAeoHtml(opts) {
  ...
  const sections = visible
    ...
    .map((o, i) => {
      ...
      const accent =
        o.type === 'schema' ? '#a78bfa' :
        o.type === 'structure' ? '#ff9f43' :
        o.type === 'meta' ? '#4dabff' :
        '#00d4aa';
      const where = o.where ? `<div style="margin:0 0 12px;font-size:12px;color:#64748b;font-style:italic;">📍 ${escapeHtmlForBlock(o.where)}</div>` : '';
      const desc = o.description ? `<div style="margin:0 0 12px;font-size:13px;color:#475569;line-height:1.5;">${escapeHtmlForBlock(o.description)}</div>` : '';
      return `
<!-- AEO opt ${i + 1} of ${visible.length} (${label}) — ${o.name || o.title || 'Optimization'} -->
<section style="margin:0 0 28px;padding:20px 22px;background:#ffffff;border:1px solid #e5e7eb;border-left:4px solid ${accent};border-radius:8px;font-family:system-ui,-apple-system,'Segoe UI',sans-serif;color:#0f172a;line-height:1.6;">
  <div style="display:inline-block;font-size:10px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:${accent};margin:0 0 8px;">${escapeHtmlForBlock(label)}</div>
  <h3 style="margin:0 0 8px;font-size:18px;font-weight:600;color:#0f172a;">${escapeHtmlForBlock(o.name || o.title || 'Optimization')}</h3>
  ${desc}
  ${where}
  <div class="aeo-opt-content">
    ${o._code}
  </div>
</section>`;
    }).join('\n');
  ...
  const schemaJoined = schemaBlocks.length
    ? `\n<!-- AEO Schema Blocks — JSON-LD for AI engines + structured data -->\n${schemaBlocks.join('\n')}\n`
    : '';

  // A wrapper with an attribution comment + minimum CSS so unstyled
  // hosts still render lists/tables/headings sensibly.
  return `<!-- =====================================================================
  AEO Optimizations — ${opts.length} block${opts.length === 1 ? '' : 's'} for this page
  Generated by Syte AEO Engine. Each <section> below is one optimization;
  paste this whole chunk wherever the AM has chosen on the page.
===================================================================== -->
<style>
  .aeo-opt-content { font-size: 15px; }
  .aeo-opt-content h2 { font-size: 22px; margin: 18px 0 10px; font-weight: 600; color: #0f172a; }
  .aeo-opt-content h3 { font-size: 17px; margin: 16px 0 8px; font-weight: 600; color: #0f172a; }
  .aeo-opt-content h4 { font-size: 15px; margin: 12px 0 6px; font-weight: 600; }
  .aeo-opt-content p  { margin: 0 0 12px; }
  .aeo-opt-content ul, .aeo-opt-content ol { margin: 0 0 14px 24px; }
  .aeo-opt-content li { margin: 4px 0; }
  .aeo-opt-content table { border-collapse: collapse; width: 100%; margin: 12px 0; font-size: 14px; }
  .aeo-opt-content th, .aeo-opt-content td { border: 1px solid #e5e7eb; padding: 8px 12px; text-align: left; }
  .aeo-opt-content th { background: #f8fafc; font-weight: 600; }
  .aeo-opt-content strong { font-weight: 600; }
  .aeo-opt-content a { color: #2563eb; text-decoration: underline; }
  .aeo-opt-content blockquote { margin: 12px 0; padding: 10px 16px; border-left: 3px solid #cbd5e1; background: #f8fafc; color: #334155; }
</style>
...
function OptPageCard({ result: r, onDelete, onVerified, optClient }) {
                ...
                {cResults.length > 0 && (
                  <>
                    ...
                    {cResults.slice(0, 5).map(r => (
                      <OptPageCard key={r.url} result={r} onDelete={() => deleteResult(r.url, c.id)} onVerified={refreshImplementations} optClient={c} />
                    ))}
                    {cResults.length > 5 && (
