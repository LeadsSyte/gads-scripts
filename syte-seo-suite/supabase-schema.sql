...
-- Run this in the Supabase SQL Editor. RLS is disabled intentionally.

create table if not exists syte_suite_clients (
...
create table if not exists syte_suite_progress (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists syte_suite_cms_queue (
  id uuid primary key default gen_random_uuid(),
  client_id uuid references syte_suite_clients(id) on delete cascade,
  module text,
  page_url text,
