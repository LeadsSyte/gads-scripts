...
{"adGroups":[{"name":"High-Intent Visitors — 30 Days","audienceList":"Service Page Visitors","audienceDuration":"30 days","targetCPM":20,"headlines":["h1","h2","h3"],"descriptions":["d1","d2"],"longHeadline":"long headline","imageConcepts":["concept1","concept2"],"callToAction":"Get a Quote"}]}`,8000);
  ...
  function buildAllCSVs(){
    ...
    function addCamp(rows,cname,budget,bidStrat,tis){const r=er();r['Campaign']=cname;r['Campaign type']='Search';r['Campaign status']='Paused';r['Campaign daily budget']=String(budget);r['Bid strategy type']=bidStrat||'Maximize conversions';if(bidStrat==='Target Impression Share'){r['Target Imp. Share']=String((tis&&tis.share)||'100');r['Target Imp. Share location']=(tis&&tis.location)||'Anywhere on results page';r['Max CPC limit']=String((tis&&tis.maxCpc)||'10.00');}r['Networks']='Google Search';r['Languages']=brief.language||'en';r['EU political ads']='No';rows.push(r);addLocRows(rows,cname,er);}
    ...
    if(g.branded){const rows=[];const cn=campName('branded');addCamp(rows,cn,getEffectiveBudget('branded'),'Target Impression Share',{share:'100',location:'Anywhere on results page',maxCpc:'10.00'});(g.branded.adGroups||[]).forEach(ag=>addAG(rows,cn,ag,'Exact'));addNegs(rows,cn,filterConflictNegs(g.branded.negatives,g.branded.adGroups));csvs.push({name:'01_Branded_Search',cols:SCOLS,rows});}
  ...
  return(
    <div style={{background:'#0f1a2a',color:'white',padding:'14px 28px',display:'flex',alignItems:'center',gap:14,position:'sticky',top:0,zIndex:50,boxShadow:'0 2px 12px rgba(0,0,0,0.3)'}}>
      ...
      <div>
        ...
        <div style={{fontSize:11,opacity:0.5}}>Multi-Campaign · DataForSEO Volume · v6.9</div>
      </div>
      {step!==undefined&&(
      ...
      <div style={{fontSize:11,background:'linear-gradient(135deg, #7c3aed, #a78bfa)',padding:'4px 10px',borderRadius:10,fontWeight:700,marginLeft:8}}>v6.9 ✨</div>
    </div>
  );
}
